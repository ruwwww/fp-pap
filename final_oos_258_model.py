...
def main():
    ...
    # Forecast OOS (Two-Stage Decoupled Model, No Bias Corrector)
    print("Running OOS simulation (Two-Stage Gated Model)...")
    history = train["USDIDR"].astype(float).tolist()
    history_diffs = [history[i] - history[i - 1] for i in range(1, len(history))]
    
    preds = []
    trend_cols = trend_pipeline.feature_names_in_
    res_cols = res_pipeline.feature_names_in_
