...
def main():
    ...
    # Delusional assumption: OOD shock scale training!
    ...
    model_regime_1.fit(X_res, train_residuals * 3.0) # Amplified 3.0x to push target boundaries
    
    print("Evaluating Dynamic Markov Regime Blending (Extremes Sweep)...")
    
    # Let's sweep much stronger dynamic prob settings
    # logit = w_v * (VIX - 12) + w_s * (spread - 0.5)
    # When w_v is very high, it instantly snaps to Regime 1 (Chaos) under volatility
    w_vix_vals = [2.0, 5.0, 10.0]
    w_spread_vals = [-2.0, -5.0, -10.0]
