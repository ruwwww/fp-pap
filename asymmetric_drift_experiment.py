...
def recursive_asymmetric_drift_forecast(
    train_df: pd.DataFrame,
    ...
    vix_fac: float,
    spread_fac: float,
    asymmetric_drift: float = 0.00015  # Additive log-return shock bias (~0.015% per day)
) -> np.ndarray:
    ...
    for i in range(len(future_df)):
        ...
        # 3. Apply Multiplicative Gates AND Additive Asymmetric Drift Bias
        ...
        is_risky = (vix_lag1 > 14.0) or (spread < 0.8)
        ...
        # Inject additive drift override during risky regimes
        if is_risky:
            ret_total += asymmetric_drift
...
def main() -> None:
    ...
    # Sweep additive drift biases
    drift_biases = [0.0, 0.00005, 0.00010, 0.00015, 0.00020, 0.00030]
    best_rmse = float("inf")
    best_drift = None
    best_preds = None
    
    print("Sweeping Asymmetric Drift Bias (Additive)...")
    for drift in drift_biases:
        preds = recursive_asymmetric_drift_forecast(train, test_exog, combined, trend_model, res_model, selected_lags, vix_fac=1.10, spread_fac=1.06, asymmetric_drift=drift)
        score = rmse(y_true, preds)
        print(f"Drift Bias: {drift:.5f} (~{drift*100:.3f}% per day) -> OOS RMSE: {score:.4f}")
        if score < best_rmse:
            best_rmse = score
            best_drift = drift
            best_preds = preds
            
    print(f"\nBest Asymmetric Drift Model RMSE: {best_rmse:.4f} (Drift={best_drift:.5f})")
    
    # Update submission if better
    if best_rmse < 269.30:
        sub_df = pd.DataFrame({
        ...
        print("New Best Score! Gated + Asymmetric Drift predictions saved to submission.csv")
    else:
