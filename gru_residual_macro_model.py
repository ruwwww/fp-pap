...
def main() -> None:
    ...
    # Convert data into 3D format for GRU (time_steps = 3 days to capture short-term memory)
    time_steps = 3
    X_gru, y_gru = build_gru_dataset(X_res_scaled_df, y_res, time_steps=time_steps)
    
    # GRU is structurally simpler than LSTM, which helps mitigate overfitting on FX noise
    ...
    gru_model.fit(X_gru, y_gru, epochs=15, batch_size=64, verbose=1)
    
    # Run GRU Forecast
    gru_preds = recursive_gru_forecast(
        train, test_exog, combined, trend_model, gru_model, scaler, selected_lags, time_steps=time_steps
    )
    gru_rmse = rmse(y_true, gru_preds)
    print(f"\nHypersimplified GRU Residual Model OOS RMSE: {gru_rmse:.4f}")
    
    # If GRU is better, update submission
    if gru_rmse < 269.30:
        sub_df = pd.DataFrame({
