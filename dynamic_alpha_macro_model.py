...
def recursive_dynamic_forecast(
    train_df: pd.DataFrame,
    future_df: pd.DataFrame,
    combined: pd.DataFrame,
    trend_models: dict[str, Pipeline],  # Dict containing different alpha trend models
    res_model,
    ...
    # We will use keys like 'low_alpha' (alpha=0.1) and 'high_alpha' (alpha=10.0)
    low_trend_model = trend_models["low"]
    high_trend_model = trend_models["high"]
    ...
    for i in range(len(future_df)):
        ...
        # Determine active trend model based on global risk state
        # Dynamic-Alpha: if VIX is high (panic), use low_alpha model to be responsive.
        # If VIX is low, use high_alpha model for stability.
        if vix_lag1 > 18.0:
            active_trend = low_trend_model
        else:
            active_trend = high_trend_model
            
        trend_cols = active_trend.feature_names_in_
        
        # 1. Dynamic Trend Prediction
        ...
        ret_trend = float(active_trend.predict(X_row_trend)[0])
...
def main() -> None:
    ...
    # 3. Forecast OOS with Dynamic-Alpha regularizer
    dyn_preds = recursive_dynamic_forecast(train, test_exog, combined, trend_models, res_model, selected_lags, vix_fac=1.10, spread_fac=1.06)
    dyn_rmse = rmse(y_true, dyn_preds)
    print(f"Dynamic-Alpha Gated Macro Model RMSE: {dyn_rmse:.4f}")
    
    # Previous best benchmark for comparison (RMSE: 269.3759)
    # Fit benchmark trend model (alpha=1.0)
    bench_trend = Pipeline([
        ("scaler", StandardScaler()),
        ("model", Ridge(alpha=1.0))
    ]).fit(X_trend, y_trend)
    
    bench_preds = recursive_dynamic_forecast(
    ...
    print(f"Static Alpha=1.0 Benchmark RMSE: {bench_rmse:.4f}")
    
    # Plot predictions
    plt.figure(figsize=(15, 6))
    plt.plot(test_actual[DATE_COL], y_true, color="black", label="Actual")
    plt.plot(test_actual[DATE_COL], bench_preds, color="blue", alpha=0.7, label=f"Static Alpha=1.0 Model (RMSE={bench_rmse:.2f})")
    plt.plot(test_actual[DATE_COL], dyn_preds, color="red", label=f"Dynamic-Alpha Model (RMSE={dyn_rmse:.2f})")
    plt.title("USDIDR Dynamic-Alpha Regularizer vs Static Benchmark")
    plt.legend()
    plt.tight_layout()
    plt.savefig("dynamic_alpha_macro_plot.png", dpi=150)
    plt.close()
