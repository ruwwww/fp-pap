...
def main():
    ...
    # Fit Residual Model with lower alpha (alpha=2.0)
    X_res_rows = []
    for t in range(start_idx, len(train)):
        row_exog = combined.iloc[t]
        feats = {
    ...
    # Dynamic VIX regime dependent dynamic scaling factor
    # We sweep smaller scale_facs to enhance fluctuations without letting the model diverge
    scale_facs = [1.0, 1.1, 1.2, 1.3, 1.4, 1.5]
    
    best_rmse = 999.0
    best_scale = 1.0
    best_preds = []
    
    for scale_fac in scale_facs:
        ...
        for i in range(len(test_exog)):
            ...
            # Apply dynamic scaling multiplier directly on the shock return component
            if vix_lag1 > 14.0:
                ret_shock *= scale_fac
        ...
        print(f"Scale: {scale_fac} -> OOS RMSE: {score:.4f}")
        if score < best_rmse:
            best_rmse = score
            best_scale = scale_fac
            best_preds = preds
            
    print(f"Sweep Finished! Best OOS RMSE: {best_rmse:.4f} with scale: {best_scale}")
    ...
    }).to_csv("continuous_dynamic_alpha_predictions.csv", index=False)
    ...
    plt.plot(test_actual["Date"], best_preds, color="red", label=f"Continuous Dynamic Model (RMSE={best_rmse:.2f})")
    plt.title(f"USDIDR Out-Of-Sample Forecasting: Dynamic Volatility Scaling (Scale={best_scale})")
    plt.legend()
    plt.savefig("continuous_dynamic_alpha_plot.png", dpi=150)
    plt.close()
