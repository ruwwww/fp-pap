#!/usr/bin/env python3
"""Strict Causal Univariate PatchTST Pipeline using Hugging Face Transformers.

Optimizations:
  - Fixed scale-mismatch drop by realigning the RevIN internal stats during inference.
  - Replaced MSE with Huber Loss to better balance macro-trend and micro-volatility.
  - Added CosineAnnealingLR scheduler to stabilize late-epoch gradient steps.
  - Injected dropout into the architecture configuration to trigger dynamic variations.
"""
...
def run_pipeline(
    ...
    config = PatchTSTConfig(
        ...
        encoder_layers=3,
        scaling="std",          # Mengaktifkan internal RevIN bawaan PatchTST
        dropout=0.2,            # Memberikan ruang bagi varians model agar tidak over-smoothing
    )
    model = PatchTSTForPrediction(config).to(DEVICE)

    # 5. Training Loop
    optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4)
    # Scheduler untuk meredam osilasi liar di epoch akhir
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS)
    # Menggunakan HuberLoss agar model lebih adaptif terhadap fluktuasi tanpa over-penalizing
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"Epoch [{epoch:02d}/{EPOCHS:02d}] - Training Loss: {epoch_loss:.6f}")

    # 6. Direct Inference Phase (Strictly Causal & Valid Scale Alignment)
    print("\nMelakukan Direct Forecasting untuk Periode Test...")
    model.eval()
    
    # Ambil data historis terakhir asli dari TRAIN set sebagai modal konteks lookback window
    # Pastikan data ini di-extract dari train_scaled asli (bebas dummy 0) agar statistik RevIN valid
    past_context = train_scaled[-LOOKBACK_WINDOW:]  # Shape: (LOOKBACK_WINDOW, 1)
    past_context_tensor = torch.tensor(past_context, dtype=torch.float32).unsqueeze(0).to(DEVICE)
    ...
    with torch.no_grad():
        ...
        # Shape output: (1, horizon, 1)
        pred_scaled = pred_outputs.squeeze(0).cpu().numpy()

    # 7. Inverse Transform Hasil Prediksi Resmi
    ...
    if actual_raw is not None:
        ...
        print(f" Optimized Univariate PatchTST RMSE    : {rmse_patchtst:.4f}")
        print(f"{'='*55}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Optimized PatchTST Hugging Face Pipeline")
