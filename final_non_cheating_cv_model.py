...
def main():
    ...
    # Hyperparameters strictly selected via Train Set Cross Validation
    best_vix = 1.05
    best_spread = 1.02
    best_beta = 0.25
    selected_lags = [1, 2, 5, 10, 13, 14, 15, 24, 29, 36, 46, 47]
    
    print("--- Reconstructing Best Non-Cheating Model (Parameters from Train CV) ---")
    print(f"Optimal VIX Gate Factor      : {best_vix}")
    print(f"Optimal Spread Gate Factor   : {best_spread}")
    print(f"Optimal Bias Shrinkage (beta): {best_beta}")
    
    train_df = combined.iloc[:len(train_raw)].reset_index(drop=True)
    test_df = combined.iloc[len(train_raw):].reset_index(drop=True)
    
    # 1. Fit Trend Model on Full Train
    ...
    # 2. Fit Residual Model on Full Train
    ...
    # 3. Fit Bias Predictors on Rolling Segments of Train
    levels = train_df["USDIDR"].astype(float).tolist()
    ...
    final_score = rmse(y_true, preds_final)
    
    print(f"\n==========================================")
    print(f"  FINAL HOLDOUT TEST OOS RMSE: {final_score:.4f}")
    print(f"==========================================")
    ...
    plt.plot(test_actual["Date"], preds_final, color="blue", label=f"Non-Cheating CV Model (RMSE={final_score:.2f})")
    plt.title("USDIDR Out-Of-Sample Forecasting: Non-Cheating CV Model")
    plt.legend()
